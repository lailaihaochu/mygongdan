...
package com.jinbang.gongdan.modules.cms.web.front;
...
import com.jinbang.gongdan.modules.cms.service.ArticleService;
import com.jinbang.gongdan.modules.cms.service.GuestbookService;
import com.jinbang.gongdan.modules.cms.service.ServiceSearch;
import com.jinbang.gongdan.modules.cms.utils.CmsUtils;
	...
	*/
	...
	public String search(String t, @RequestParam(required=false) String q, @RequestParam(required=false) String qand, @RequestParam(required=false) String qnot, 
		...
		if ("cmd:reindex".equals(q)){
			if (UserUtils.getUser().isAdmin()){
				...
				if (StringUtils.isBlank(t) || "article".equals(t)){
					serviceSearch.createIndex();
				}
				// 留言模型
				else if ("guestbook".equals(t)){
					serviceSearch.createIndexGuestBook();
				}
		...
		// 执行检索
			...
			// 文章检索
			if (StringUtils.isBlank(t) || "article".equals(t)){
				Page<Article> page = serviceSearch.getArticles(new Page<Article>(request, response), qStr);
				page.setMessage("匹配结果，共耗时 " + (System.currentTimeMillis() - start) + "毫秒。");
				model.addAttribute("page", page);
			}
			// 留言检索
			else if ("guestbook".equals(t)){
				Page<Guestbook> page = serviceSearch.getGuestBook(new Page<Guestbook>(request, response), qStr);
				page.setMessage("匹配结果，共耗时 " + (System.currentTimeMillis() - start) + "毫秒。");
				model.addAttribute("page", page);
			}
